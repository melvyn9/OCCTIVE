...
import { CustomNode } from './NodeShapes';
...
const DependencyGraph: React.FC<DependencyGraphProps> = ({ flowId, highlightId }) => {
  ...
  const graphNodes = positioned.map((n) => {
    ...
    */
    ...
    const shapeOfGeneration = new Map([
      ['O.1', 'rectangle'],
      ['O.2', 'diamond'],
    ]);

    const shape = shapeOfGeneration.get(n.data.generation);
    const color = colourOf.get(n.data.group);
    ...
    return {
      ...
      data: {
        ...n.data,
        label: isTarget ? `⭐ ${n.data.label}` : n.data.label,
        color,
        shape,
      },
      /* style: { display: 'none' }, */
  ...
  return (
    <ReactFlowProvider>
      ...
      <div style={{ height: 520, border: '1px solid #d1d5db' }}>
        <ReactFlow id={id} nodes={graphNodes} edges={edges} fitView nodeTypes={{ custom: CustomNode }}>
          <Background />
        </ReactFlow>
      </div>
    </ReactFlowProvider>
